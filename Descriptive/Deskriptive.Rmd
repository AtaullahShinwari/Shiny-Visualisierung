...
library(modelr)
library(purrr)
#Split Dataset into 5 sets, with training and test data
cv  <- crossv_kfold(heart, k = 5)

clf1 <- lm(data=heart)  # build linear regression model on full data
clf2 <- svm(Age ~ ., data = heart)
clf3 <- randomForest(HeartDisease ~ .,data=heart, importance=TRUE,proximity=TRUE)
clf1
clf2
clf3
